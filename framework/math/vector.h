...
struct Vec2T {
    ...
    Vec2T<T> operator+(const Vec2T<T>& rhs) const { return Vec2T(x + rhs.x, y + rhs.y); }
    ...
    Vec2T<T> operator*(const Vec2T<T>& rhs) const { return Vec2T<T>(x * rhs.x, y * rhs.y); }
    Vec2T<T>& operator*=(const Vec2T<T>& rhs) {
        x *= rhs.x;
        y *= rhs.y;
        return *this;
...
struct Vec3T {
    ...
    Vec3T<T> operator+(const Vec3T<T>& rhs) const { return Vec3T<T>(x + rhs.x, y + rhs.y, z + rhs.z); }
...
struct Vec4T {
    ...
    Vec4T<T> operator+(const Vec4T<T>& rhs) const { return Vec4T<T>(x + rhs.x, y + rhs.y, z + rhs.z, w + rhs.w); }
